class DesignDrawingsController < ApplicationController
  ...
  def index
    @drawings = DesignDrawing.includes(:design_project).all
  end

  def show
    @drawing = DesignDrawing.includes(:design_project, :drawing_versions).find(params[:id])
    @versions = @drawing.drawing_versions.order(version_number: :desc)
  ...
  def create
    ...
    if @drawing.save
      if params[:file]
        create_version(@drawing, params[:file])
      end
  ...
  def create_version
    ...
    if params[:file].is_a?(ActionDispatch::Http::UploadedFile) && params[:file].original_filename.present?
      ...
      if version.save
        redirect_to design_drawing_path(@drawing), notice: "版本 #{version.version_number} 上传成功"
      else
